...
7. Restart WSGI and require `/healthz` to return `ok: true` with the exact process-captured `releaseSha`.
...
9. Serve content-versioned committed `static/` assets and run the notification worker separately when background Notifications are enabled.
