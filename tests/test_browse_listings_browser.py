...
window.tmdbGetDiscoverPage=async(path,params)=>({page:params.page,total_pages:params.page===1?2:2,results:params.page===1?(params.sort_by==='primary_release_date.asc'?raw.slice().reverse():raw):[{id:3,title:'Third movie',release_date:'2009-01-01'}]});
...
document.querySelector('.browse-menu-sort summary').click();button('Release Date — Oldest').click();await flush();check(cards()[0].textContent.includes('Second movie'),'provider sort request and returned order');
document.querySelector('.browse-menu-service summary').click();document.querySelector('.browse-service-option').click();await flush();check(document.body.dataset.route.includes('provider=8'),'service route');
document.querySelector('.browse-menu-other summary').click();const company=document.querySelector('[aria-label="Search production companies"]');company.value='Studio';company.dispatchEvent(new Event('input',{bubbles:true}));await new Promise(r=>setTimeout(r,300));button('Synthetic company').click();await flush();check(document.body.dataset.route.includes('company=9'),'picker route');
...
button('CLEAR ALL').click();await flush();check(!document.body.dataset.route.includes('company='),'clear company filter');
