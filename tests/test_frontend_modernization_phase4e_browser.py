...
class FrontendModernizationPhase4EBrowserTests(unittest.TestCase):
    @staticmethod
    ...
    def dump_dom(self, browser: str, url: str) -> str:
        with tempfile.TemporaryDirectory(prefix="tv-tracker-phase4e-browser-") as profile_dir:
    ...
    def test_danger_settings_vue_mounts_existing_destructive_contract_in_real_browser(self):
        ...
        if browser is None:
            ...
            if in_ci:
                self.fail("CI must provide Chromium/Chrome for the Phase 4E Danger Zone Vue E2E")
            self.skipTest("Chromium/Chrome is not installed in this local test environment")

        account = {
            "username": "phase4e-admin",
            "password_hash": "unused",
            "session_version": 1,
            "updated_at": None,
        }
        tracker_data = {
            "shows": {"101": {"name": "Phase 4E Show", "status": "watching"}},
            "movies": {},
            "history": [{"show_id": "101", "season": 1, "episode": 1}],
            "profile": {
                "username": "Phase 4E Danger",
                "favorite_shows": ["101"],
                "favorite_movies": [],
                "avatar_type": "initial",
        ...
        with patch.object(tracker, "ensure_schema", return_value=None), patch.object(
            ...
            @app.get("/__phase4e_danger")
            def phase4e_danger():
                session["authenticated"] = True
                session["session_version"] = 1
                session["csrf_token"] = "phase4e-browser-csrf"
                return redirect("/app/settings/danger-zone")
            ...
            try:
                dom = self.dump_dom(browser, f"{base_url}/__phase4e_danger")
                self.assertIn(
                    'meta name="app-route" content="/app/settings/danger-zone"',
                    dom,
                )
                self.assertIn(
                    'data-tvtracker-vue-danger-settings="danger-zone"',
                    dom,
                    "the manifest-driven Vue bundle must replace the legacy Danger Zone shell",
                )
                self.assertIn('id="reset-data-button"', dom)
                self.assertIn("Reset Tracker Data", dom)
                self.assertIn("Deactivate account", dom)
                self.assertIn("Delete account", dom)
                self.assertGreaterEqual(dom.count(" disabled"), 2)
                self.assertIn("Available when user accounts are enabled.", dom)
