...
assert(component.includes('v-if="model.state === \'loading\'"'));
assert(component.includes('class="watchlist-initial-skeleton upcoming-initial-skeleton"'));
...
assert(bridge.includes('const loading = items.length === 0 && (startBackgroundRefresh || global.isRefreshingUpcoming === true)'));
assert(bridge.includes('global.getUpcomingShows()'));
assert(bridge.includes('function prepareUpcomingDisplayItems(groupItems)'));
assert(bridge.includes('function getUpcomingBatchKey(show,episode)'));
assert(bridge.includes('const displays = prepareUpcomingDisplayItems(groupItems);'));
assert(!bridge.includes('global.prepareUpcomingDisplayItems'));
...
assert(!ui.includes('function prepareUpcomingDisplayItems(groupItems)'));
