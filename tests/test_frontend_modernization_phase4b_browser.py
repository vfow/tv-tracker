...
class FrontendModernizationPhase4BBrowserTests(unittest.TestCase):
    @staticmethod
    ...
    def dump_dom(self, browser: str, url: str) -> str:
        with tempfile.TemporaryDirectory(prefix="tv-tracker-phase4b-browser-") as profile_dir:
    ...
    def test_profile_settings_vue_mounts_legacy_service_contract_in_real_browser(self):
        ...
        if browser is None:
            ...
            if in_ci:
                self.fail("CI must provide Chromium/Chrome for the Phase 4B Profile Vue E2E")
            self.skipTest("Chromium/Chrome is not installed in this local test environment")

        account = {
            "username": "phase4b-admin",
        ...
        with patch.object(tracker, "ensure_schema", return_value=None), patch.object(
            ...
            @app.get("/__phase4b_auth")
            def phase4b_auth():
                session["authenticated"] = True
                session["session_version"] = 1
                session["csrf_token"] = "phase4b-browser-csrf"
            ...
            try:
                dom = self.dump_dom(browser, f"{base_url}/__phase4b_auth")
                self.assertIn(
                ...
                self.assertIn(
                    ...
                    dom,
                    "the manifest-driven Vue bundle must replace the legacy Profile shell",
                )
                self.assertIn('id="profile-username-input"', dom)
                self.assertIn('id="settings-avatar-preview"', dom)
                self.assertIn('id="profile-header-preview-wrap"', dom)
                self.assertIn('id="upload-profile-avatar"', dom)
                self.assertIn('id="upload-profile-header"', dom)
                self.assertIn('id="save-profile-settings"', dom)
                self.assertIn('id="adult-filter-input"', dom)
