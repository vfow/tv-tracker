...
class TMDBOnlyContractTests(unittest.TestCase):
    ...
    def test_movie_genres_people_media_and_discover_gradients_exist(self):
        ...
        self.assertIn('["Info","Cast","Crew","Details","Genres","Releases","Where to Watch"]', ui)
    ...
    def test_phase_63b_media_routes_and_country_filtering_exist(self):
        ...
        self.assertIn('cleanMedia === "movie" ? "?media=movie" : ""', app)
        ...
        self.assertIn('getPersonDetailRoute("person",pageState.personId,personRouteLabel,"movie")', ui)
        ...
        self.assertIn('return candidate + ("?media=movie" if media_type == "movie" else "")', backend)
    ...
    def test_streaming_availability_lazy_refresh_and_id_genres_exist(self):
        app = self.read('static/js/app.js')
        router = self.read('static/js/app-router.js')
        ui = self.read('static/js/ui.js')
        tmdb = self.read('static/js/tmdb.js')
        ...
        self.assertIn('async function tmdbGetTitleWatchProviders(media,titleId,options={})', tmdb)
        self.assertNotIn('"watch/providers",', tmdb.split('const TMDB_SHOW_APPEND_TO_RESPONSE = [',1)[1].split('];',1)[0])
        ...
        self.assertIn('Where to Watch', ui)
        self.assertIn('No watch options available in your region.', ui)
        self.assertIn('Powered by JustWatch', ui)
        self.assertIn('VIEW WATCH OPTIONS', ui)
        self.assertIn('loadDetailWatchProviders("tv",showId,{force:true})', ui)
        self.assertIn('loadDetailWatchProviders("movie",moviePageState.movie.id,{force:true})', app)
        ...
        self.assertIn('watch-provider-logo-tile', source_css)
        self.assertIn('watch-provider-logo-tile', built_css)
